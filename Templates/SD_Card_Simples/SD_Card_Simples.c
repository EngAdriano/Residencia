...
static void display_values(float temp, float hum, bool recording) {
    ...
    ssd1306_draw_string(0, 0, "Datalogger AHT10");
...
int main(void) {
    ...
    // temporizadores
    const uint32_t display_interval_ms = 2000;
    const uint32_t log_interval_ms = 60 * 1000;

    absolute_time_t next_display = make_timeout_time_ms(200);
    ...
    while (1) {
        ...
        if (absolute_time_diff_us(now, next_log) >= 0) {
            ...
            sleep_ms(700);
            display_values(temp, hum, false);
            next_log = delayed_by_ms(next_log, log_interval_ms);
        }
