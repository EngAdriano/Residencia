...
void ssd1306_init(ssd1306_t *dev, i2c_inst_t *i2c, uint sda, uint scl) {
    dev->i2c = i2c; dev->sda = sda; dev->scl = scl;
    ...
    ssd1306_command(dev, 0xAE); ssd1306_command(dev, 0xD5); ssd1306_command(dev, 0x80);
    ssd1306_command(dev, 0xA8); ssd1306_command(dev, SSD1306_HEIGHT-1);
    ssd1306_command(dev, 0xD3); ssd1306_command(dev, 0x00); ssd1306_command(dev, 0x40);
    ssd1306_command(dev, 0x8D); ssd1306_command(dev, 0x14); ssd1306_command(dev, 0x20); ssd1306_command(dev,0x00);
    ssd1306_command(dev,0xA1); ssd1306_command(dev,0xC8); ssd1306_command(dev,0xDA); ssd1306_command(dev,0x12);
    ssd1306_command(dev,0x81); ssd1306_command(dev,0xCF); ssd1306_command(dev,0xD9); ssd1306_command(dev,0xF1);
    ssd1306_command(dev,0xDB); ssd1306_command(dev,0x40); ssd1306_command(dev,0xA4); ssd1306_command(dev,0xA6); ssd1306_command(dev,0xAF);
}

void ssd1306_clear(ssd1306_t *dev) { memset(dev->buffer,0,sizeof(dev->buffer)); }

void ssd1306_show(ssd1306_t *dev){
    for(uint8_t page=0; page<SSD1306_HEIGHT/8; page++){
        ssd1306_command(dev,0xB0+page);
        ssd1306_command(dev,0x00); ssd1306_command(dev,0x10);
        uint8_t data[SSD1306_WIDTH+1]; data[0]=0x40;
...
void ssd1306_draw_pixel(ssd1306_t *dev,uint8_t x,uint8_t y,bool color){
    ...
    if(color) dev->buffer[x+(y/8)*SSD1306_WIDTH]|=1<<(y%8);
