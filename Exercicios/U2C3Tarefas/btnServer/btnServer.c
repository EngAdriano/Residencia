...
void start_server();    // Inicializa o servidor
err_t http_server_accept(void *arg, struct tcp_pcb *newpcb, err_t err); // Handler de conexão
err_t http_server_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *pbuffer, err_t err); // Handler de requisições


// Descrição: Este programa implementa um servidor HTTP simples que monitora o estado de um botão conectado ao GPIO 5.
int main() {
