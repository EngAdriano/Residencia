...
{ 
  ...
  USART_Cmd(USART3, ENABLE);  //使能串口3 
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=0;//抢占优先级4
			...
			case 2:
			{
				...
				if(Usart3_RxCounter==24)
				{
					...
					if(HLW8032_SumCheck==Rec_Data)  //校验
					{
						...
						DisplayTable[0]=(float)Voltage_Parameter_REG*3.006/Voltage_REG; //电压
...
//						else
						{
							DisplayTable[1]=(float)Current_Parameter_REG/Current_REG;  //电流
							DisplayTable[2]=(float)Power_Parameter_REG*3.006/Power_REG;//功率
